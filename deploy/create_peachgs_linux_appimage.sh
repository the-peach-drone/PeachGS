...
if [[ $# -eq 0 ]]; then
  echo 'create_peachgs_linux_appimage.sh PGS_SRC_DIR PGS_RELEASE_DIR'
  exit 1
fi

QGC_SRC=$(readlink -f $1)

QGC_CUSTOM_APP_NAME="${QGC_CUSTOM_APP_NAME:-Peach-GS}"
QGC_CUSTOM_GENERIC_NAME="${QGC_CUSTOM_GENERIC_NAME:-Ground Control Station}"
QGC_CUSTOM_BINARY_NAME="${QGC_CUSTOM_BINARY_NAME:-Peach-GS}"
QGC_CUSTOM_LINUX_START_SH="${QGC_CUSTOM_LINUX_START_SH:-${QGC_SRC}/deploy/Peach-GS-start.sh}"
QGC_CUSTOM_APP_ICON="${QGC_CUSTOM_APP_ICON:-${QGC_SRC}/resources/icons/PLogoFull.png}"
QGC_CUSTOM_APP_ICON_NAME="${QGC_CUSTOM_APP_ICON_NAME:-PeachGS}"
...
# copy PeachGS release into appimage
