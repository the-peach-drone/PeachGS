...
void MockLinkFTP::_openCommand(uint8_t senderSystemId, uint8_t senderComponentId, MavlinkFTP::Request* request, uint16_t seqNumber)
{
    ...
    } else if (_BinParamFileEnabled && path == "@PARAM/param.pck?withdefaults=1") {
    ...
    response.openFileLength = (path == "@PARAM/param.pck?withdefaults=1" ? 1024*1024 : _currentFile.size());
