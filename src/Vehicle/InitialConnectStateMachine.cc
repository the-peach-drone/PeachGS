...
void InitialConnectStateMachine::_stateRequestMission(StateMachine* stateMachine)
{
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isPX4Flow() || sharedLink->isLogReplay()) {
            ...
            connect(vehicle->_missionManager, &MissionManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
        }
    }
}
...
void InitialConnectStateMachine::_stateRequestGeoFence(StateMachine* stateMachine)
{
    ...
    disconnect(vehicle->_missionManager, &MissionManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isPX4Flow() || sharedLink->isLogReplay()) {
            ...
            if (vehicle->_geoFenceManager->supported()) {
                ...
                connect(vehicle->_geoFenceManager, &GeoFenceManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
            } else {
...
void InitialConnectStateMachine::_stateRequestRallyPoints(StateMachine* stateMachine)
{
    ...
    if(vehicle->_geoFenceManager->supported() && vehicle->_geoFenceManager != nullptr) {
        disconnect(vehicle->_geoFenceManager, &GeoFenceManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    }
    ...
    if (!sharedLink) {
        ...
        if (sharedLink->linkConfiguration()->isHighLatency() || sharedLink->isPX4Flow() || sharedLink->isLogReplay()) {
            ...
            if (vehicle->_rallyPointManager->supported()) {
                ...
                connect(vehicle->_rallyPointManager, &RallyPointManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
            } else {
...
void InitialConnectStateMachine::_stateSignalInitialConnectComplete(StateMachine* stateMachine)
{
    ...
    if(vehicle->_rallyPointManager->supported() && vehicle->_rallyPointManager != nullptr) {
        disconnect(vehicle->_rallyPointManager, &RallyPointManager::progressPct, connectMachine, &InitialConnectStateMachine::gotProgressUpdate);
    }
